    ...
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 dark:from-slate-900 dark:via-indigo-950 dark:to-slate-900 overflow-hidden relative">
        ...
        <div class="w-full sm:max-w-md mt-6 px-10 py-12 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl shadow-2xl overflow-hidden sm:rounded-3xl border border-white/20 dark:border-slate-700/50 relative z-10 transition-all duration-500">
            <div class="text-center mb-10">
                ...
                <h2 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter uppercase italic">Atur Ulang Sandi</h2>
                <p class="text-slate-500 dark:text-slate-400 font-bold italic mt-2">Buat kata sandi baru untuk akun Anda</p>
            </div>

            <form method="POST" action="{{ route('password.store') }}" class="space-y-6 italic">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="email" class="block w-full bg-slate-50/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-indigo-500/20 rounded-2xl py-4" type="email" name="email" :value="old('email', $request->email)" required autofocus autocomplete="username" />
                    <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>

                <!-- Password -->
                <div class="space-y-2">
                    <x-input-label for="password" :value="__('Kata Sandi Baru')" class="font-black text-slate-700 dark:text-slate-300" />
                    <x-text-input id="password" class="block w-full bg-slate-50/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-indigo-500/20 rounded-2xl py-4" type="password" name="password" required autocomplete="new-password" placeholder="••••••••" />
                    <x-input-error :messages="$errors->get('password')" class="mt-2" />
                </div>

                <!-- Confirm Password -->
                <div class="space-y-2">
                    <x-input-label for="password_confirmation" :value="__('Konfirmasi Kata Sandi')" class="font-black text-slate-700 dark:text-slate-300" />
                    <x-text-input id="password_confirmation" class="block w-full bg-slate-50/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-indigo-500/20 rounded-2xl py-4" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="••••••••" />
