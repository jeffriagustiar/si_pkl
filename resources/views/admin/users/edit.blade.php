            ...
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-xl sm:rounded-2xl border border-slate-200 dark:border-slate-700">
                <div class="p-8 text-slate-900 dark:text-slate-100 italic">
                    <div class="mb-8 border-b dark:border-slate-700 pb-4">
                        ...
                        <p class="text-slate-400 dark:text-slate-500 text-sm mt-1 italic font-medium">Perbarui informasi akun untuk {{ $user->name }}.</p>
                    </div>
                    ...
                    <form action="{{ route('admin.users.update', $user) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 focus:ring-indigo-500 rounded-xl" type="text" name="name" :value="old('name', $user->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="email" :value="__('Alamat Email')" class="font-bold text-slate-700 dark:text-slate-300 italic" />
                                <x-text-input id="email" class="block mt-1 w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 focus:ring-indigo-500 rounded-xl" type="email" name="email" :value="old('email', $user->email)" required />
                            ...
                            <div>
                                ...
                                <select id="role" name="role" class="block mt-1 w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 focus:ring-indigo-500 rounded-xl italic">
                                    ...
                                    <option value="teacher" {{ $user->role == 'teacher' ? 'selected' : '' }}>Pembimbing</option>
                            ...
                            <div class="pt-4 border-t dark:border-slate-700">
                                <x-input-label for="password" :value="__('Ganti Kata Sandi (Biarkan kosong jika tidak diganti)')" class="font-bold text-slate-700 dark:text-slate-300 italic" />
                                <x-text-input id="password" class="block mt-1 w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 focus:ring-indigo-500 rounded-xl" type="password" name="password" />
